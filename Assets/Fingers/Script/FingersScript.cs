...
namespace DigitalRubyShared
{
    public class FingersScript : MonoBehaviour
    {
        ...
        [Tooltip("Shows a circle over each touch point - only shown in Unity editor.")]
        public GameObject[] TouchCircles;

        [Tooltip("Whether to show touches")]
        ...
        private CaptureResult ShouldCaptureGesture(GameObject o)
        {
            ...
            // top level text always passes through
            if (o.GetComponent<Text>() != null && o.transform.parent.GetComponent<Canvas>() != null)
            {
                return CaptureResult.ForcePassThrough;
            }
            // scroll bars never pass through and stop the loop
            else if (o.GetComponent<Scrollbar>() != null)
            {
        ...
        private void PopulateGameObjectsForTouch(int pointerId, float x, float y)
        {
            ...
            {
                result = ShouldCaptureGesture(r.gameObject);
                if (result == CaptureResult.ForcePassThrough)
                {
                    list.Clear();
                    break;
                }
                else if (result == CaptureResult.ForceDenyPassThrough)
                {
